...
1. Write a module rule that wraps your tool and integrates its input and output into the pipeline. 
    ...
    * If you're adding new tools from an existing YAML, use ``conda env update --file configs/conda/camp_short-read-assembly.yaml --prune``.
...
3. If applicable, update the default conda config using ``conda env export > config/conda/camp_short-read-assembly.yaml`` with your tool and its dependencies. 
